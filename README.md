...
2) to train the model using the data in `flask/lib/data/train.tsv` run: `docker run -it -v $PWD/flask/lib:/app/lib -p 5000:5000 sentiment_api python3 app.py --host=0.0.0.0`
3) to start the api and make it available to host run: `docker run -it -v $PWD/flask/lib:/app/lib sentiment_api python3 app.py --host=0.0.0.0`
...
- if you want to create a start script:
    - on Linux: `echo "docker run -it -v $PWD/flask/lib:/app/lib -p 5000:5000 sentiment_api" > start && chmod +x start`
